...
<script>(function($){var list=$('#content').find('.cell');if(list.length>0){var txt='';$('#content').find('.cell').each(function(i){if(i===0){txt+='<li class="active">1</li>'}else{txt+='<li>'+(i+1)+'</li>'}});$('#indicator').html(txt);var w_w=$(window).width();setTimeout(function(){new C_Scroll({container:'slide',content:'content',ct:'indicator',size:w_w,intervalTime:5000,lazyIMG:!!0})},20);setTimeout(function(){$('#slide').show()},20)}})(jQuery);</script>
    ...
    <div id="tab_01" class="newsct">
        ...
        <div>
            ...
            <?php $index_topinfo = mymps_get_infos(10,NULL,3,NULL,NULL,NULL,NULL,NULL,$cityid); ?>            <?php if(is_array($index_topinfo)){foreach($index_topinfo as $k => $mymps) { ?>            <li style="<? if($mymps['ifbold'] == 1) { ?>font-weight:bold;<?php } if($mymps['ifred'] == 1) { ?>color:red;<?php } ?>">
            ...
            <?php $news = mymps_get_news(7,NULL,NULL,NULL,NULL,NULL,$cityid); ?>            <?php if(is_array($news)){foreach($news as $k => $mymps) { ?>            <li style="<? if($mymps['is_commend'] == 1) { ?>color:red;<?php } ?>">
            ...
            <?php } else { ?>
            <ul class="list_normal first_bold tab-cont" style="display:none;">
            <?php $newinfo = mymps_get_infos(6,NULL,NULL,NULL,NULL,NULL,NULL,NULL,$city['cityid']); ?>            <?php if(is_array($newinfo)){foreach($newinfo as $k => $mymps) { ?>            <li style="<? if($mymps['ifred'] == 1) { ?>color:red;<?php } if($mymps['ifbold'] == 1) { ?>font-weight:bold;<?php } ?>">
            <font class="time">[<? echo GetTime($mymps['begintime'],'m-d'); ?>]</font> 
            ...
            <?php $members = mymps_get_members(10,NULL,NULL,NULL,2,NULL,NULL,$cityid); ?>            <ul class="list_normal first_bold tab-cont" style="display:none;">
            <?php if(is_array($members)){foreach($members as $k => $mymps) { ?>            <li><img src="<?=$mymps_global['SiteUrl']?><?=$mymps['prelogo']?>" class="img"> <a href="index.php?mod=store&id=<?=$mymps['id']?>"><?=$mymps['tname']?></a><span class="jian"></span></li>
            <?php }} ?>
