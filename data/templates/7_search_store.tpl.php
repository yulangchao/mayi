...
<div class="c"></div>
 	<div class="main2 cc" id="mainbox">
    	<div class="h">
            ...
            <span class="mr20"><a href="#"><?=$mymps_global['SiteName']?>搜索</a> &raquo; 商家</span><span class="s6">小提示：多个词用空格分隔，搜索的更准！</span>
        </div>
...
<div class="content fl">
        	<div class="s_dlA">
                ...
                <?php if(is_array($search['result'])){foreach($search['result'] as $k => $result) { ?>                <dl>
                    <dt>
                      <a  href="<?=$result['uri']?>" target="_blank"><img src="<?=$mymps_global['SiteUrl']?><?=$result['prelogo']?>"></a>
                        ...
                        联系电话：<?=$result['tel']?> <br />联系地址：<?=$result['address']?><br>
                        </div>
                    </dt>
                    <dd>
                        <a href="<?=$result['uri_aboutus']?>" target="_blank">商家简介</a>
                        <a href="<?=$result['uri_comment']?>" target="_blank">添加点评</a>
                        <a href="<?=$result['uri_album']?>" target="_blank">商家相册</a>
                    </dd>
                </dl>
                <?php }} ?>
    			</div>
...
<div style="display:none;">
                    ...
                    <dd>
                    <select name="catid">
                    	<option value="" <? if(!$catid) { ?>style="background-color:#6eb00c; color:white!important;"<?php } ?>>不限类别</option><?php if(is_array($catoption)){foreach($catoption as $k => $options) { ?>                        <option value="<?=$options['corpid']?>" <? if($options['corpid'] == $catid) { ?>selected style="background-color:#6eb00c; color:white!important;"<?php } ?>>><?=$options['corpname']?></option>
                        <?php }} ?>
                    </select>
</dd>
                    <dt>所在地区：</dt>
