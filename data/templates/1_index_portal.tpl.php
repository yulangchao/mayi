...
<div class="rollBox">
        ...
        <div id="ad_indexcatad_<?=$portaluri['catidershou']?>"></div>
        <div class="ershou">
            <div class="hd">
                <div class="span">二手转让</div>
                <div class="center"></div>
                <div class="more"><a href="<?=$portaluri['moreershou']?>" target="_blank">更多</a></div>
            </div>
            <div class="bd">
                <div class="ershouleft">
                    <ul>
                        <?php if(is_array($ershou_img)){foreach($ershou_img as $mymps) { ?>                        <li><a href="<?=$mymps['uri']?>" title="<?=$mymps['title']?>" target="_blank"><img src="<?=$mymps_global['SiteUrl']?><?=$mymps['img_path']?>"></a></li>
                        <?php }} ?>
                    </ul>
                </div>
                <div class="ershouright">
                    <ul>
                        <?php if(is_array($ershou)){foreach($ershou as $mymps) { ?>                        <li>[<?=$mymps['catname']?>] <a href="<?=$mymps['uri']?>" target="_blank" style="<? if($mymps['ifred'] == 1) { ?>color:red;<?php } if($mymps['ifbold'] == 1) { ?>font-weight:bold;<?php } ?>" title="<?=$mymps['title']?>"><?=$mymps['title']?></a> </li>
                        <?php }} ?>
                    </ul>
                </div>
            </div>
        ...
        <div class="clearfix"></div>
        <div id="ad_indexcatad_<?=$portaluri['catidzufang']?>"></div>
        <div class="house">
            <div class="zufang">
                <div class="hd">
                    <div class="span">二手房</div>
                    <div class="center"></div>
                    <div class="more"><a href="<?=$portaluri['moreershoufang']?>" target="_blank">更多</a></div>
                </div>
                <div class="bd">
                    <ul>
                        <?php if(is_array($esf)){foreach($esf as $mymps) { ?>                        <li>
                            <div class="img"><a href="<?=$mymps['uri']?>" target="_blank" title="<?=$mymps['title']?>"><img src="<?=$mymps_global['SiteUrl']?><? echo $mymps['img_path'] ? $mymps['img_path'] : '/images/nophoto.jpg'; ?>" alt="<?=$mymps['title']?>"/></a></div>
                            <div class="des"><a href="<?=$mymps['uri']?>" target="_blank"><?=$mymps['title']?></a></div>
                            <div class="rent"><?=$mymps[$tpl_index['portali']['prices']]?>万元/<?=$mymps[$tpl_index['portali']['acreage']]?>平米</div>
                        </li>
                        <?php }} ?>
                    </ul>
                </div>
            ...
            <div class="ershoufang">
                <div class="hd">
                    <div class="span">租房</div>
                    <div class="more"><a href="<?=$portaluri['morezufang']?>" target="_blank">更多</a></div>
                </div>
                <div class="bd">
                    <ul>
                        <?php if(is_array($czf)){foreach($czf as $mymps) { ?>                        <li>
                            <div class="title"><a href="<?=$mymps['uri']?>" title="<?=$mymps['title']?>" target="_blank"><? echo cutstr($mymps['title'],23); ?></a></div>
                        </li>
                        <?php }} ?>
                    </ul>
                </div>
        ...
        <div id="ad_indexcatad_<?=$portaluri['catidershoufang']?>"></div>
        <div id="ad_indexcatad_<?=$portaluri['catidzhaopin']?>"></div>
        <div class="clearfix"></div>
        <div class="zhaopin_jianli">
            <div class="zhaopin">
                <div class="hd">
                    <div class="span">招聘信息</div>
                    <div class="center"></div>
                    <div class="more"><a href="<?=$portaluri['morezhaopin']?>" target="_blank">更多</a></div>
                </div>
                <div class="bd">
                    <ul>
                        <?php if(is_array($zhaopin)){foreach($zhaopin as $mymps) { ?>                        <li style="<? if($mymps['ifred'] == 1) { ?>color:red;<?php } if($mymps['ifbold'] == 1) { ?>font-weight:bold;<?php } ?>"><a href="<?=$mymps['uri']?>" target="_blank"><?=$mymps['title']?>&nbsp;&nbsp;&nbsp;&nbsp;[<?=$mymps[$tpl_index['portali']['company']]?>]</a></li>
                        <?php }} ?>
                    </ul>
                </div>
            </div>
            <div class="jianli">
                <div class="hd">
                    ...
                    <div class="more"><a href="<?=$portaluri['morejianli']?>" target="_blank">更多</a></div>
                </div>
                <div class="bd">
                    <ul>
                    <?php if(is_array($jianli)){foreach($jianli as $mymps) { ?>                    <li><a href="<?=$mymps['uri']?>">[<?=$mymps['contact_who']?>] <?=$mymps['title']?></a></li>
                    <?php }} ?>
                    </ul>
                </div>
                <div class="clear"></div>
                <div><a href="<?=$portaluri['morezhaopin']?>" target="_blank"><img src="<?=$mymps_global['SiteUrl']?>/template/default/images/index/qiuzhi.gif" align="absmiddle" border="0"></a></div>
           </div>
        </div>
        <div class="clearfix"></div>
        <div id="ad_indexcatad_<?=$portaluri['catidjianli']?>"></div>
        <?php if(ifplugin('goods')){$goods = mymps_get_goods($tpl_index['goods'],1,NULL,NULL,NULL,NULL,$cityid); ?><div class="clear5"></div>
<div class="goods">
        	<div class="ul">
                <div class="hd">
                ...
                <div class="bd">
                    ...
                    <a href="<?=$mymps['uri']?>"  target=_blank><img src="<?=$mymps_global['SiteUrl']?>/<?=$mymps['picture']?>" title="<?=$mymps['goodsname']?>"/>
...
<div class="flink">
        	<div class="ul">
                ...
                <? if($friendlink['img']) { ?>
                <ul class="image">
                <?php if(is_array($friendlink['img'])){foreach($friendlink['img'] as $mymps) { ?>                <li><a href="<?=$mymps['url']?>" target="_blank" title="<?=$mymps['name']?>"><img src="<?=$mymps_global['SiteUrl']?><?=$mymps['logo']?>" border="0" /></a></li>
                <?php }} ?>
                </ul>
                <?php } ?>
                <? if($friendlink['txt']) { ?>
