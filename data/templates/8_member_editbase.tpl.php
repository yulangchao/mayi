...
<form name="form1" method="post" action="index.php?mod=member&action=editbase&dopost=1" onSubmit="return chkSubmit();">
   ...
   <?php $row['prelogo'] = strstr($row['prelogo'],'http') ? $row['prelogo'] : $mymps_global['SiteUrl'].$row['prelogo']; ?>       <dd><img src="<? echo $row['prelogo'] ? $row['prelogo'] : 'template/images/center_ico.png'; ?>" id="Image1"  width="60" height="60" /> <a href="index.php?mod=member&action=upface" style="color:#36c; float:right;">修改</a></dd>
      </dl>
      <dl class="cfix">
       <dt>用户ID</dt>
       <dd><?=$row['id']?></dd>
      ...
      <dl class="cfix">
       ...
       <dd><input id="mobile" type="text" name="mobile" value="<?=$row['mobile']?>" placeholder="输入手机号码" /></dd>
      </dl>
    </div>
    <? if($row['if_corp'] == 1) { ?>
    ...
    <div class="inp_Itembox">
      ...
      <dl class="cfix">
       ...
       <dd>
       <? echo select_where_option('/include/selectwhere.php',$row['cityid'],$row['areaid'],$row['streetid']);; ?>       </dd>
       <div class="menu">
        <i></i>
        <i></i>
        <i></i>
