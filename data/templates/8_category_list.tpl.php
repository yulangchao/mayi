...
<div class="filter2" id="filter2">
        ...
        <div class="inner" style="display:none;">
            <ul>
        	<li><a href="index.php?mod=category&cityid=<?=$cityid?>&catid=<?=$catid?>" <? if(!$areaid) { ?>class='selected'<?php } ?>>不限</a></li>
        <?php if(is_array($city['area'])){foreach($city['area'] as $fcat) { ?>            <li class="item" id="cat_<?=$fcat['areaid']?>">
                ...
                <ul id="items<?=$fcat['areaid']?>">
                	<li><a href="index.php?mod=category&cityid=<?=$cityid?>&catid=<?=$catid?>&areaid=<?=$fcat['areaid']?>" <? if(!$streetid) { ?>class='selected'<?php } ?>>不限</a></li>
                    <?php if(is_array($fcat['street'])){foreach($fcat['street'] as $scat) { ?>                    <li><a href='index.php?mod=category&catid=<?=$catid?>&cityid=<?=$cityid?>&areaid=<?=$fcat['areaid']?>&streetid=<?=$scat['streetid']?>' data-id='<?=$scat['streetid']?>'  id='s_b_<?=$scat['streetid']?>' <? if($streetid == $scat['streetid']) { ?>class='selected'<?php } ?>><?=$scat['streetname']?></a></li>
                    <?php }} ?>
                </ul>
                <?php } else { ?>
                <ul id="items<?=$fcat['areaid']?>">
        ...
        <div class="inner" style="display:none;">
            <ul>
            <?php if(is_array($hotcities)){foreach($hotcities as $k => $mymps) { ?>            <a href="index.php?mod=category&catid=<?=$catid?>&cityid=<?=$mymps['cityid']?>"><?=$mymps['cityname']?></a>
            <?php }} ?> 
            </ul>
        </div>
        <?php } ?>
...
<dd class="attr pr5">
                    ...
                    <span class="price">
                    <? echo substr($w,0,1) === '0' ? ' 面议 ' : str_replace('.00','',$w); ?>                    </span>
                    <?php } ?>
                    <?php }} ?>
