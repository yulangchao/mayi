...
<span class="attrVal"  style="<? if(strexists($mymps['value'],'元')) { ?>color:#ff7800;font-weight:bold;font-size:20px;<?php } ?>"><? if(in_array($mymps['value'],array('0元','0元/月','0元/平米/天','0万元','0.00元','0.00万元'))) { ?>面议<?php } else { echo str_replace('.00','',$mymps['value']); ?><?php } ?></span>
</li>
            <?php } ?>
<?php }} ?>
</ul>
...
<div class="follow">
<ul>
            <?php $relevant = mymps_get_infos(6,NULL,NULL,NULL,$row['catid'],NULL,NULL,NULL,$cityid); ?>            <?php if(is_array($relevant)){foreach($relevant as $mymps) { ?><li><a href="index.php?mod=information&id=<?=$mymps['id']?>"><? echo cutstr($mymps['title'],26); ?></a><span><? echo get_format_time($mymps['begintime']); ?></span></li>
<?php }} ?>
</ul>
