...
<script type="text/javascript">uaredirect("<?=$mymps_global['SiteUrl']?>/m/index.php?mod=news&catid=<?=$catid?>&cityid=<?=$city['cityid']?>");</script>
...
<ul class="subcls schitem"> 
                        <?php if(is_array($cat_list)){foreach($cat_list as $mymps) { ?><li <? if($mymps['catid'] == $channel['catid']) { ?>class='thiscls'<?php } ?>><a href="<?=$mymps['uri']?>">全部</a></li>
                        <?php if(is_array($mymps['children'])){foreach($mymps['children'] as $w) { ?>                        <li <? if($w['catid'] == $channel['catid']) { ?>class='thiscls'<?php } ?>><a href="<?=$w['uri']?>"><?=$w['catname']?></a></li> 
                        <?php }} ?>
<?php }} ?>
</ul> 
</div>
...
<div class="nowlist">
<ul>
                        <?php $i=1; ?>                            <?php if(is_array($news)){foreach($news as $mymps) { ?><li><span class="title"><a href="<?=$mymps['uri']?>" class="title" target="_blank" <? if($mymps['iscommend'] == 1) { ?>style="color:red"<?php } ?>><?=$mymps['title']?></a></span><span class="time"><? echo GetTime($mymps['begintime'],'y-m-d'); ?></span></li>
<? if($i%5 == 0) { ?><li style="background:none"></li><?php } ?>
                            <?php $i++; ?>                            <?php }} ?>
</ul>
</div>
